...
public class Debugger {
    ...
    public static void debugE(String Tag, String Text) {
        if (isDebugMode) Log.e(Tag, Text + "");
        if (isFileDebugMode) addLogDetails(Tag + " :: " + Text + "\n");
    }

    public static void debugI(String Tag, String Text) {
        if (isDebugMode) Log.i(Tag, Text + "");
        if (isFileDebugMode) addLogDetails(Tag + " :: " + Text + "\n");
    }

    public static void debugD(String Tag, String Text) {
        if (isDebugMode) Log.d(Tag, Text + "");
        if (isFileDebugMode) addLogDetails(Tag + " :: " + Text + "\n");
    }
    ...
    public static File getDir() {
        if (dir == null) {
            ...
            if (android.os.Environment.getExternalStorageState().equals(android.os.Environment.MEDIA_MOUNTED)) {
                dir = new File(android.os.Environment.getExternalStorageDirectory(), "VidStatus2k18");
